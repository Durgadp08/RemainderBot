...
const promptToSend = (task: string) => {
  const timeandDate = dayjs().utc().format("YYYY-MM-DD HH:mm");
  const promptAI = `
  You are a smart assistant that extracts structured tasks from natural language.
  
  **Current Date and Time for reference: ${timeandDate}**
